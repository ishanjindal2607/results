...
def sram_traffic(
    ...
    max_parallel_window = 2 * max_parallel_window
    ...
    for v in tqdm(range(int(num_v_fold))):
        ...
        if num_h_fold > 1:
            ...
            parallel_window = 2 * int(math.ceil(cols_this_fold / dimension_cols))
    ...
    return (str(cycles), avg_util, total_MAC)
...
def gen_trace_filter(
                    cycle = 0,
                    num_filters = 4, parallel_window = 2,
                    window_size = 27,
                    num_rows = 4, num_cols =4,
